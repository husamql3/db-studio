...
- **Server sidecar**: `src/server/server-process.ts` spawns the built CLI (`packages/server/dist/index.js`) with `--database-url`, a free `PORT`, `HOST=127.0.0.1`, and `ALLOWED_ORIGINS` set to the renderer origin. One connection at a time; switching kills and respawns, then reloads the window. Packaged builds run it with `ELECTRON_RUN_AS_NODE=1` against `resources/server` (staged by `scripts/stage-server.mjs`, which `npm install`s the server's runtime deps; `better-sqlite3` ships N-API prebuilds so no Electron rebuild is needed). Dev uses the system `node`.
