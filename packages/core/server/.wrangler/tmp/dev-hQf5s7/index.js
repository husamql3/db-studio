...
// src/dao/create-table.dao.ts
var createTable = /* @__PURE__ */ __name(async (tableData) => {
...
// src/dao/delete-records.dao.ts
var getForeignKeyReferences = /* @__PURE__ */ __name(async (tableName) => {
  const query = `
		SELECT
			tc.constraint_name,
...
var getRelatedRecords = /* @__PURE__ */ __name(async (tableName, primaryKeys) => {
  ...
  for (const [_tableColumn, constraints] of constraintsByTable) {
    ...
    const placeholders = pkValues.map((_2, i) => `$${i + 1}`).join(", ");
    const query = `
...
var deleteRecords = /* @__PURE__ */ __name(async (params) => {
...
var forceDeleteRecords = /* @__PURE__ */ __name(async (params) => {
  const { tableName, primaryKeys } = params;
  const client = await db.connect();
  try {
    await client.query("BEGIN");
    ...
    const deleteRelatedRecursively = /* @__PURE__ */ __name(async (targetTable, targetColumn, values) => {
      const nestedFks = await getForeignKeyReferences(targetTable);
      for (const nestedFk of nestedFks) {
        const placeholders3 = values.map((_2, i) => `$${i + 1}`).join(", ");
        const selectQuery = `
					SELECT "${nestedFk.referencedColumn}" FROM "${targetTable}"
					WHERE "${targetColumn}" IN (${placeholders3})
				`;
      ...
      const placeholders2 = values.map((_2, i) => `$${i + 1}`).join(", ");
      const deleteQuery = `
				DELETE FROM "${targetTable}"
				WHERE "${targetColumn}" IN (${placeholders2})
			`;
    ...
    const message = totalRelatedDeleted > 0 ? `Successfully deleted ${mainDeleted} ${mainDeleted === 1 ? "record" : "records"} from "${tableName}" and ${totalRelatedDeleted} related ${totalRelatedDeleted === 1 ? "record" : "records"} from other tables` : `Successfully deleted ${mainDeleted} ${mainDeleted === 1 ? "record" : "records"} from "${tableName}"`;
    return {
      success: true,
      message,
...
// src/dao/insert-record.dao.ts
var insertRecord = /* @__PURE__ */ __name(async (params) => {
  const { tableName, data } = params;
  const client = await db.connect();
  try {
    const columns = Object.keys(data);
    const values = Object.values(data);
    const placeholders = columns.map((_2, index) => `$${index + 1}`).join(", ");
...
function mapPostgresToDataType(pgType) {
  const normalized = pgType.toLowerCase().trim();
  if (normalized.startsWith("array") || normalized.includes("[]") || normalized === "date" || normalized === "time" || normalized === "time without time zone" || normalized.startsWith("time(") || normalized === "timestamp" || normalized === "timestamp without time zone" || normalized.startsWith("timestamp(") || normalized === "timestamp with time zone" || normalized === "timestamptz" || normalized.startsWith("timestamp with time zone(")) {
    return DataTypes.long;
  }
  if (normalized === "integer" || normalized === "int" || normalized === "int4" || normalized === "bigint" || normalized === "int8" || normalized === "smallint" || normalized === "int2" || normalized === "decimal" || normalized.startsWith("decimal(") || normalized === "numeric" || normalized.startsWith("numeric(") || normalized === "real" || normalized === "float4" || normalized === "double precision" || normalized === "float8" || normalized === "float" || normalized === "serial" || normalized === "serial4" || normalized === "bigserial" || normalized === "serial8" || normalized === "money") {
  ...
  if (normalized === "character varying" || normalized.startsWith("varchar") || normalized.startsWith("character varying(") || normalized === "character" || normalized.startsWith("char") || normalized.startsWith("character(") || normalized === "bpchar" || normalized === "uuid" || normalized === "interval" || normalized.startsWith("interval") || normalized === "bytea" || normalized === "point" || normalized === "line" || normalized === "polygon" || normalized === "inet" || normalized === "cidr" || normalized === "macaddr" || normalized === "macaddr8") {
    return DataTypes.short;
  }
  return DataTypes.short;
}
__name(mapPostgresToDataType, "mapPostgresToDataType");
var StandardizedDataType = {
  // Numeric types
  int: "int",
  bigint: "bigint",
  smallint: "smallint",
...
function standardizeDataTypeLabel(pgType) {
  const normalized = pgType.toLowerCase().trim();
  if (normalized === "integer" || normalized === "int" || normalized === "int4" || normalized === "serial" || normalized === "serial4") {
  ...
  if (normalized === "decimal" || normalized.startsWith("decimal(") || normalized === "numeric" || normalized.startsWith("numeric(")) {
  ...
  if (normalized === "character varying" || normalized.startsWith("varchar") || normalized.startsWith("character varying(")) {
    return StandardizedDataType.varchar;
  }
  if (normalized === "character" || normalized.startsWith("char") || normalized.startsWith("character(") || normalized === "bpchar") {
  ...
  if (normalized === "timestamp" || normalized === "timestamp without time zone" || normalized.startsWith("timestamp(")) {
    return StandardizedDataType.timestamp;
  }
  if (normalized === "timestamp with time zone" || normalized === "timestamptz" || normalized.startsWith("timestamp with time zone(")) {
...
__name(standardizeDataTypeLabel, "standardizeDataTypeLabel");

// src/dao/table-columns.dao.ts
var getTableColumns = /* @__PURE__ */ __name(async (tableName) => {
  const client = await db.connect();
  try {
...
// src/dao/table-list.dao.ts
var getTablesList = /* @__PURE__ */ __name(async () => {
  const client = await db.connect();
  try {
    const res = await client.query(`
      SELECT 
...
// src/dao/tables-data.dao.ts
var buildWhereClause = /* @__PURE__ */ __name((filters) => {
  if (filters.length === 0) {
...
var getTableData = /* @__PURE__ */ __name(async (tableName, page = 1, pageSize = 50, sort = "", order = "asc", filters = []) => {
...
// src/dao/update-records.dao.ts
var updateRecords = /* @__PURE__ */ __name(async (params) => {
...
app.post("/tables", async (c) => {
  try {
    ...
    const errorDetail = error && typeof error === "object" && "detail" in error ? error.detail : void 0;
    return c.json(
      {
        success: false,
...
app.delete("/records/force", async (c) => {
  try {
    ...
    return c.json(
      {
        success: false,
        message: error instanceof Error ? error.message : "Failed to force delete records",
        detail: errorDetail
      },
      500
    );
...
// .wrangler/tmp/bundle-9PPqCr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-9PPqCr/middleware-loader.entry.ts
