...
export const useColumnPreferences = ({
	...
	const { prefsByTable, setColumnPrefs, clearColumnPrefs } = useColumnPreferencesStore();
	...
	// The store keeps every table's preferences, so switching scope is just a
	// different lookup — no reload, and every consumer re-renders together.
	const prefs = prefsByTable[makeColumnPrefKey(key)] ?? EMPTY_COLUMN_PREFS;
	...
	// identity; the no-op guard breaks any save→re-render cycle.
	...
	useEffect(() => {
		...
		const current = getColumnPrefs(key);
		const reconciled = reconcileColumnPrefs(schemaColsRef.current, current);
		if (sameColumnPrefs(current, reconciled)) return;
		setColumnPrefs(key, reconciled);
	}, [key, schemaKey, setColumnPrefs]);

	// Writes go through the same no-op guard: an update that reconciles to the
	// stored state does not touch the store and so does not re-render.
	const update = useCallback(
		(next: ColumnPrefs) => {
			const reconciled = reconcileColumnPrefs(schemaColsRef.current, next);
			if (sameColumnPrefs(getColumnPrefs(key), reconciled)) return;
			setColumnPrefs(key, reconciled);
		},
		[key, setColumnPrefs],
	);

	const reconciledPrefs = useCallback(
		() => reconcileColumnPrefs(schemaColsRef.current, getColumnPrefs(key)),
		[key],
	);

	const toggleColumn = useCallback(
		(columnName: string) => {
			const reconciled = reconciledPrefs();
