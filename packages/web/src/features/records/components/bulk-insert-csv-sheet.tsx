...
export const BulkInsertCsvSheet = ({ tableName }: { tableName: string }) => {
		...
		try {
			await bulkInsertRecords(parsedRecords, {
				onSuccess: () => {
					...
					setParsedRecords([]);
					setParseError(null);
					closeOverlay("records.bulk-insert-csv");
				},
			});
		} catch (error) {
	...
	return (
		<SheetSidebar
			title={`Add data from CSV into: ${tableName}`}
			open={isOverlayOpen("records.bulk-insert-csv")}
			onOpenChange={(open) => {
				if (!open) {
					handleCancel();
				}
		...
		>
			<div className="flex flex-col h-full gap-4">
				<Alert
					variant="info"
					title="CSV Import"
					message="Enter CSV data with headers. Supports comma, tab, or semicolon delimiters. First row should be column names."
				/>
