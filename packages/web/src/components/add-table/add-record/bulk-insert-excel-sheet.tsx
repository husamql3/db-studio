...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@db-studio/ui/tabs";
...
interface BulkInsertExcelSheetProps {
	tableName: string;
}

export const BulkInsertExcelSheet = ({ tableName }: BulkInsertExcelSheetProps) => {
	...
	return (
		<SheetSidebar
			title={`Add data from Excel into: ${tableName}`}
			open={isSheetOpen("bulk-insert-excel")}
			onOpenChange={(open) => {
				if (!open) {
					handleCancel();
				}
		...
		>
			<div className="flex flex-col h-full gap-4">
				...
				<Alert
					variant="info"
					title="Excel Import"
					message="Upload an Excel file (.xlsx, .xls) with headers in the first row. Column names should match your table schema."
				/>
